// Email Service para automatización de hosting
...
class EmailService {
  ...
  generateWelcomeEmail(credentials: AccountCredentials): EmailTemplate {
...
www.lanzawebar.com
  ...
  async sendWelcomeEmail(credentials: AccountCredentials): Promise<boolean> {
    try {
...
www.lanzawebar.com
  ...
  async sendRegistrationWelcomeEmail(userData: { name: string; email: string }): Promise<boolean> {
    try {
            ...
            <div class="content">
                ...
                <div class="config-box">
                    ...
                    ${nameservers.map((ns, index) => 
                      `<div class="nameserver-item">NS${index + 1}: ${ns}</div>`
                    ).join('')}
                </div>
